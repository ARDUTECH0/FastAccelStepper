...
- fix: those same `extern` declarations lived in RampGenerator.h, included *after* RampCalculator.h - but RampCalculator.h's own inline methods use them as soon as that header is parsed, before RampGenerator.h's declarations are ever seen. Moved into RampCalculator.h itself, in the branch that defines SUPPORT_LOG2_TIMER_FREQ_VARIABLES
- fix: in that same fallback path, `log2_timer_freq_div_sqrt_of_2` was computed with a copy-paste of the formula for `log2_timer_freq_square_div_2` (squaring the tick frequency instead of dividing it by sqrt(2)), making the two identical. This produced absurdly large (multi-billion-tick, i.e. minutes-long) results from `calculate_ticks()`/`calculate_ramp_steps()`'s non-cubic branch - reachable on every ramp start/reversal from standstill unless `setLinearAcceleration()` is in use (s_h > 0 routes small step counts through the unaffected cubic branch instead), which is what made this so easy to have missed: any move that happens to use linear acceleration never triggers it
